...
@ApiTags("ContractType Query")
@UseGuards(ContractTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("contracttypes/query")
export class ContractTypeQueryController {
  #logger = new Logger(ContractTypeQueryController.name);

  constructor(private readonly service: ContractTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all contracttype with optional pagination" })
  @ApiResponse({ status: 200, type: ContractTypesResponse })
  @ApiQuery({ name: "options", required: false, type: ContractTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ContractType>    
  ): Promise<ContractTypesResponse<ContractType>> {
    try {
     
      const contracttypes = await this.service.findAll(options);
      logger.info("Retrieving all contracttype");
  ...
  @ApiOperation({ summary: "Find contracttype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter contracttype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ContractTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ContractType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find contracttypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ContractType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all contracttypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count contracttypes with conditions" })
  @ApiResponse({ status: 200, type: ContractTypesResponse<ContractType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ContractTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one contracttype with conditions" })
  ...
  @ApiOperation({ summary: "Find one contracttype or return error" })
  @ApiResponse({ status: 200, type: ContractTypeResponse<ContractType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ContractType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get contracttype by ID" })
  @ApiResponse({ status: 200, type: ContractTypeResponse<ContractType> })
  @ApiResponse({ status: 404, description: "ContractType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the contracttype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ContractTypeResponse<ContractType>> {
    try {
      const contracttype = await this.service.findOne({ where: { id } });
      if (!contracttype) {
        throw new NotFoundException(
          "ContractType no encontrado para el id solicitado"
        );
      }
      return contracttype;
    } catch (error) {
