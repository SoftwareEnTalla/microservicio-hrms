...
@ApiTags("AccessLevel Query")
@UseGuards(AccessLevelAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("accesslevels/query")
export class AccessLevelQueryController {
  #logger = new Logger(AccessLevelQueryController.name);

  constructor(private readonly service: AccessLevelQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all accesslevel with optional pagination" })
  @ApiResponse({ status: 200, type: AccessLevelsResponse })
  @ApiQuery({ name: "options", required: false, type: AccessLevelDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AccessLevel>    
  ): Promise<AccessLevelsResponse<AccessLevel>> {
    try {
     
      const accesslevels = await this.service.findAll(options);
      logger.info("Retrieving all accesslevel");
      return accesslevels;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find accesslevel by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter accesslevel', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AccessLevelsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AccessLevel no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find accesslevels with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AccessLevel>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all accesslevels" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count accesslevels with conditions" })
  @ApiResponse({ status: 200, type: AccessLevelsResponse<AccessLevel> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one accesslevel with conditions" })
  ...
  @ApiOperation({ summary: "Find one accesslevel or return error" })
  @ApiResponse({ status: 200, type: AccessLevelResponse<AccessLevel> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get accesslevel by ID" })
  @ApiResponse({ status: 200, type: AccessLevelResponse<AccessLevel> })
  @ApiResponse({ status: 404, description: "AccessLevel not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the accesslevel to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AccessLevelResponse<AccessLevel>> {
    try {
      const accesslevel = await this.service.findOne({ where: { id } });
      if (!accesslevel) {
        throw new NotFoundException(
