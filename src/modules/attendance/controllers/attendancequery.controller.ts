...
import { AttendanceDto } from "../dtos/all-dto";
...
@ApiTags("Attendance Query")
@UseGuards(AttendanceAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("attendances/query")
export class AttendanceQueryController {
  #logger = new Logger(AttendanceQueryController.name);

  constructor(private readonly service: AttendanceQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all attendance with optional pagination" })
  @ApiResponse({ status: 200, type: AttendancesResponse })
  @ApiQuery({ name: "options", required: false, type: AttendanceDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Attendance>    
  ): Promise<AttendancesResponse<Attendance>> {
    try {
     
      const attendances = await this.service.findAll(options);
  ...
  @ApiOperation({ summary: "Find attendance by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter attendance', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AttendancesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Attendance no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find attendances with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Attendance>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all attendances" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count attendances with conditions" })
  @ApiResponse({ status: 200, type: AttendancesResponse<Attendance> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Attendances no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one attendance with conditions" })
  ...
  @ApiOperation({ summary: "Find one attendance or return error" })
  @ApiResponse({ status: 200, type: AttendanceResponse<Attendance> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<AttendanceResponse<Attendance> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Attendance no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the attendance to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AttendanceResponse<Attendance>> {
    try {
      const attendance = await this.service.findOne({ where: { id } });
      if (!attendance) {
        throw new NotFoundException(
          "Attendance no encontrado para el id solicitado"
        );
      }
      return attendance;
    } catch (error) {
