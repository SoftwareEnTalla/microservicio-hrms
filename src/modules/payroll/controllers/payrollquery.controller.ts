...
import { PayrollDto } from "../dtos/all-dto";
...
@ApiTags("Payroll Query")
@UseGuards(PayrollAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("payrolls/query")
export class PayrollQueryController {
  #logger = new Logger(PayrollQueryController.name);

  constructor(private readonly service: PayrollQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all payroll with optional pagination" })
  @ApiResponse({ status: 200, type: PayrollsResponse })
  @ApiQuery({ name: "options", required: false, type: PayrollDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Payroll>    
  ): Promise<PayrollsResponse<Payroll>> {
    try {
     
      const payrolls = await this.service.findAll(options);
      logger.info("Retrieving all payroll");
      return payrolls;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find payroll by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter payroll', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PayrollsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Payroll no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find payrolls with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Payroll>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all payrolls" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count payrolls with conditions" })
  @ApiResponse({ status: 200, type: PayrollsResponse<Payroll> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Payrolls no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one payroll with conditions" })
  ...
  @ApiOperation({ summary: "Find one payroll or return error" })
  @ApiResponse({ status: 200, type: PayrollResponse<Payroll> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Payroll no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the payroll to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PayrollResponse<Payroll>> {
    try {
      const payroll = await this.service.findOne({ where: { id } });
      if (!payroll) {
        throw new NotFoundException(
          "Payroll no encontrado para el id solicitado"
        );
      }
      return payroll;
    } catch (error) {
