...
import { AccessControlDto } from "../dtos/all-dto";

import { logger } from '@core/logs/logger';

@ApiTags("AccessControl Query")
@UseGuards(AccessControlAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("accesscontrols/query")
export class AccessControlQueryController {
  #logger = new Logger(AccessControlQueryController.name);

  constructor(private readonly service: AccessControlQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all accesscontrol with optional pagination" })
  @ApiResponse({ status: 200, type: AccessControlsResponse })
  @ApiQuery({ name: "options", required: false, type: AccessControlDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AccessControl>    
  ): Promise<AccessControlsResponse<AccessControl>> {
    try {
     
      const accesscontrols = await this.service.findAll(options);
      logger.info("Retrieving all accesscontrol");
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get accesscontrol by ID" })
  @ApiResponse({ status: 200, type: AccessControlResponse<AccessControl> })
  @ApiResponse({ status: 404, description: "AccessControl not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the accesscontrol to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AccessControlResponse<AccessControl>> {
    try {
      const accesscontrol = await this.service.findOne({ where: { id } });
      if (!accesscontrol) {
  ...
  @ApiOperation({ summary: "Find accesscontrol by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter accesscontrol', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AccessControlsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AccessControl no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find accesscontrols with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AccessControl>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all accesscontrols" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count accesscontrols with conditions" })
  @ApiResponse({ status: 200, type: AccessControlsResponse<AccessControl> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one accesscontrol with conditions" })
  @ApiResponse({ status: 200, type: AccessControlResponse<AccessControl> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one accesscontrol or return error" })
  @ApiResponse({ status: 200, type: AccessControlResponse<AccessControl> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
