...
@ApiTags("LeaveRequestStatus Query")
@UseGuards(LeaveRequestStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("leaverequeststatuss/query")
export class LeaveRequestStatusQueryController {
  #logger = new Logger(LeaveRequestStatusQueryController.name);

  constructor(private readonly service: LeaveRequestStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all leaverequeststatus with optional pagination" })
  @ApiResponse({ status: 200, type: LeaveRequestStatussResponse })
  @ApiQuery({ name: "options", required: false, type: LeaveRequestStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<LeaveRequestStatus>    
  ): Promise<LeaveRequestStatussResponse<LeaveRequestStatus>> {
    try {
     
      const leaverequeststatuss = await this.service.findAll(options);
      logger.info("Retrieving all leaverequeststatus");
  ...
  @ApiOperation({ summary: "Find leaverequeststatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter leaverequeststatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LeaveRequestStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "LeaveRequestStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find leaverequeststatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<LeaveRequestStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all leaverequeststatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count leaverequeststatuss with conditions" })
  @ApiResponse({ status: 200, type: LeaveRequestStatussResponse<LeaveRequestStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one leaverequeststatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one leaverequeststatus or return error" })
  @ApiResponse({ status: 200, type: LeaveRequestStatusResponse<LeaveRequestStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get leaverequeststatus by ID" })
  @ApiResponse({ status: 200, type: LeaveRequestStatusResponse<LeaveRequestStatus> })
  @ApiResponse({ status: 404, description: "LeaveRequestStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the leaverequeststatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LeaveRequestStatusResponse<LeaveRequestStatus>> {
    try {
      const leaverequeststatus = await this.service.findOne({ where: { id } });
