 ...
 * Licencia: MIT
 *
 * OnHrmsPermissionsSourceChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `hrms-permissions-source` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'hrms-permissions-source';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'hrms-permissions-source-created',
  'hrms-permissions-source-updated',
  'hrms-permissions-source-deleted',
];

@Injectable()
export class OnHrmsPermissionsSourceChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnHrmsPermissionsSourceChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnHrmsPermissionsSourceChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnHrmsPermissionsSourceChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onHrmsPermissionsSourceChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
