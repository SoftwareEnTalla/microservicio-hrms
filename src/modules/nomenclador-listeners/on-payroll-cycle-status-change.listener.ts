 ...
 * Licencia: MIT
 *
 * OnPayrollCycleStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payroll-cycle-status` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payroll-cycle-status';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'payroll-cycle-status-created',
  'payroll-cycle-status-updated',
  'payroll-cycle-status-deleted',
];

@Injectable()
export class OnPayrollCycleStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPayrollCycleStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPayrollCycleStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPayrollCycleStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPayrollCycleStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
