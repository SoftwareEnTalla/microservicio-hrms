 ...
 * Licencia: MIT
 *
 * OnAttendanceChannelChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `attendance-channel` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'attendance-channel';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'attendance-channel-created',
  'attendance-channel-updated',
  'attendance-channel-deleted',
];

@Injectable()
export class OnAttendanceChannelChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnAttendanceChannelChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnAttendanceChannelChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnAttendanceChannelChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onAttendanceChannelChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
