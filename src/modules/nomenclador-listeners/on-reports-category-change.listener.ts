 ...
 * Licencia: MIT
 *
 * OnReportsCategoryChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `reports-category` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'reports-category';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  ...
  'reports-category-updated',
  'reports-category-deleted',
];

@Injectable()
export class OnReportsCategoryChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnReportsCategoryChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnReportsCategoryChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnReportsCategoryChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onReportsCategoryChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
