 ...
 * Licencia: MIT
 *
 * OnTimeEntryTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `time-entry-type` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'time-entry-type-created',
  'time-entry-type-updated',
  'time-entry-type-deleted',
];

@Injectable()
export class OnTimeEntryTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnTimeEntryTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnTimeEntryTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnTimeEntryTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onTimeEntryTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
