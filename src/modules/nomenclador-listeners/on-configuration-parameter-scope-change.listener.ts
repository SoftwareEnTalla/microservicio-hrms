 ...
 * Licencia: MIT
 *
 * OnConfigurationParameterScopeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `configuration-parameter-scope` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'configuration-parameter-scope';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'configuration-parameter-scope-created',
  'configuration-parameter-scope-updated',
  'configuration-parameter-scope-deleted',
];

@Injectable()
export class OnConfigurationParameterScopeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnConfigurationParameterScopeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnConfigurationParameterScopeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnConfigurationParameterScopeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onConfigurationParameterScopeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
