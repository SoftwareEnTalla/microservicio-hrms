 ...
 * Licencia: MIT
 *
 * OnPersonAttributeScopeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `person-attribute-scope` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'person-attribute-scope';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'person-attribute-scope-created',
  'person-attribute-scope-updated',
  'person-attribute-scope-deleted',
];

@Injectable()
export class OnPersonAttributeScopeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPersonAttributeScopeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPersonAttributeScopeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPersonAttributeScopeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPersonAttributeScopeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
