 ...
 * Licencia: MIT
 *
 * OnAccessCredentialTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `access-credential-type` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'access-credential-type';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'access-credential-type-created',
  'access-credential-type-updated',
  'access-credential-type-deleted',
];

@Injectable()
export class OnAccessCredentialTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnAccessCredentialTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnAccessCredentialTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnAccessCredentialTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onAccessCredentialTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
