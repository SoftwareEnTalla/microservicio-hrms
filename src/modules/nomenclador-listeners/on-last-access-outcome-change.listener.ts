 ...
 * Licencia: MIT
 *
 * OnLastAccessOutcomeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `last-access-outcome` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'last-access-outcome';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'last-access-outcome-created',
  'last-access-outcome-updated',
  'last-access-outcome-deleted',
];

@Injectable()
export class OnLastAccessOutcomeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnLastAccessOutcomeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnLastAccessOutcomeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnLastAccessOutcomeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onLastAccessOutcomeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
