 ...
 * Licencia: MIT
 *
 * OnPayrollFrequencyChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payroll-frequency` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payroll-frequency';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'payroll-frequency-created',
  'payroll-frequency-updated',
  'payroll-frequency-deleted',
];

@Injectable()
export class OnPayrollFrequencyChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPayrollFrequencyChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPayrollFrequencyChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPayrollFrequencyChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPayrollFrequencyChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
