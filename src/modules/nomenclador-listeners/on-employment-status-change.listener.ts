 ...
 * Licencia: MIT
 *
 * OnEmploymentStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `employment-status` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'employment-status';
const OWNER_BC = 'hrms';
...
@Injectable()
export class OnEmploymentStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnEmploymentStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnEmploymentStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnEmploymentStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onEmploymentStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
