 ...
 * Licencia: MIT
 *
 * OnLeaveRequestStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `leave-request-status` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'leave-request-status';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'leave-request-status-created',
  'leave-request-status-updated',
  'leave-request-status-deleted',
];

@Injectable()
export class OnLeaveRequestStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnLeaveRequestStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnLeaveRequestStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnLeaveRequestStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onLeaveRequestStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
