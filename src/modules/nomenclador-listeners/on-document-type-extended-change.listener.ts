 ...
 * Licencia: MIT
 *
 * OnDocumentTypeExtendedChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `document-type-extended` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'document-type-extended';
...
const TOPICS: string[] = [
  'document-type-extended-created',
  'document-type-extended-updated',
  'document-type-extended-deleted',
];

@Injectable()
export class OnDocumentTypeExtendedChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnDocumentTypeExtendedChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnDocumentTypeExtendedChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnDocumentTypeExtendedChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onDocumentTypeExtendedChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
