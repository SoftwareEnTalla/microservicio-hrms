 ...
 * Licencia: MIT
 *
 * OnTrainingModalityChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `training-modality` (owner: hrms). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'training-modality';
const OWNER_BC = 'hrms';
...
const TOPICS: string[] = [
  'training-modality-created',
  'training-modality-updated',
  'training-modality-deleted',
];

@Injectable()
export class OnTrainingModalityChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnTrainingModalityChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnTrainingModalityChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnTrainingModalityChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onTrainingModalityChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
