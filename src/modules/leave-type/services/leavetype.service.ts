...
export class LeaveTypeService {
  constructor(private readonly repository: LeaveTypeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<LeaveType[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<LeaveType[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<LeaveType[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<LeaveType | null> {
  ...
  async update(id: string, partialEntity: Partial<LeaveType>): Promise<LeaveType | null> {
