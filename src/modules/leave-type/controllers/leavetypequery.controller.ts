...
import { LeaveTypeDto } from "../dtos/all-dto";
...
@ApiTags("LeaveType Query")
@UseGuards(LeaveTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("leavetypes/query")
export class LeaveTypeQueryController {
  #logger = new Logger(LeaveTypeQueryController.name);

  constructor(private readonly service: LeaveTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all leavetype with optional pagination" })
  @ApiResponse({ status: 200, type: LeaveTypesResponse })
  @ApiQuery({ name: "options", required: false, type: LeaveTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<LeaveType>    
  ): Promise<LeaveTypesResponse<LeaveType>> {
    try {
     
      const leavetypes = await this.service.findAll(options);
      logger.info("Retrieving all leavetype");
      return leavetypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find leavetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter leavetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LeaveTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "LeaveType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find leavetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<LeaveType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all leavetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count leavetypes with conditions" })
  @ApiResponse({ status: 200, type: LeaveTypesResponse<LeaveType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades LeaveTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one leavetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one leavetype or return error" })
  @ApiResponse({ status: 200, type: LeaveTypeResponse<LeaveType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad LeaveType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get leavetype by ID" })
  @ApiResponse({ status: 200, type: LeaveTypeResponse<LeaveType> })
  @ApiResponse({ status: 404, description: "LeaveType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the leavetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LeaveTypeResponse<LeaveType>> {
    try {
      const leavetype = await this.service.findOne({ where: { id } });
      if (!leavetype) {
        throw new NotFoundException(
          "LeaveType no encontrado para el id solicitado"
        );
      }
      return leavetype;
    } catch (error) {
