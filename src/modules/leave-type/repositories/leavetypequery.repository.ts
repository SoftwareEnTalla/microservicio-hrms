...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class LeaveTypeQueryRepository {

    //Constructor del repositorio de datos: LeaveTypeQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination LeaveType on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[LeaveType[], number]> {
      logger.info('Ready to findAndCount LeaveType on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of LeaveType on repository:',result);
      return result;
    }
