...
import { PayrollFrequencyDto } from "../dtos/all-dto";
...
@ApiTags("PayrollFrequency Query")
@UseGuards(PayrollFrequencyAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("payrollfrequencys/query")
export class PayrollFrequencyQueryController {
  #logger = new Logger(PayrollFrequencyQueryController.name);

  constructor(private readonly service: PayrollFrequencyQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all payrollfrequency with optional pagination" })
  @ApiResponse({ status: 200, type: PayrollFrequencysResponse })
  @ApiQuery({ name: "options", required: false, type: PayrollFrequencyDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PayrollFrequency>    
  ): Promise<PayrollFrequencysResponse<PayrollFrequency>> {
    try {
     
      const payrollfrequencys = await this.service.findAll(options);
      logger.info("Retrieving all payrollfrequency");
  ...
  @ApiOperation({ summary: "Find payrollfrequency by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter payrollfrequency', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PayrollFrequencysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PayrollFrequency no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find payrollfrequencys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PayrollFrequency>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all payrollfrequencys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count payrollfrequencys with conditions" })
  @ApiResponse({ status: 200, type: PayrollFrequencysResponse<PayrollFrequency> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PayrollFrequencys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one payrollfrequency with conditions" })
  ...
  @ApiOperation({ summary: "Find one payrollfrequency or return error" })
  @ApiResponse({ status: 200, type: PayrollFrequencyResponse<PayrollFrequency> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PayrollFrequency no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get payrollfrequency by ID" })
  @ApiResponse({ status: 200, type: PayrollFrequencyResponse<PayrollFrequency> })
  @ApiResponse({ status: 404, description: "PayrollFrequency not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the payrollfrequency to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PayrollFrequencyResponse<PayrollFrequency>> {
    try {
      const payrollfrequency = await this.service.findOne({ where: { id } });
      if (!payrollfrequency) {
        throw new NotFoundException(
          "PayrollFrequency no encontrado para el id solicitado"
        );
      }
