...
@ApiTags("ScopeType Query")
@UseGuards(ScopeTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("scopetypes/query")
export class ScopeTypeQueryController {
  #logger = new Logger(ScopeTypeQueryController.name);

  constructor(private readonly service: ScopeTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all scopetype with optional pagination" })
  @ApiResponse({ status: 200, type: ScopeTypesResponse })
  @ApiQuery({ name: "options", required: false, type: ScopeTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ScopeType>    
  ): Promise<ScopeTypesResponse<ScopeType>> {
    try {
     
      const scopetypes = await this.service.findAll(options);
      logger.info("Retrieving all scopetype");
      return scopetypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find scopetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter scopetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ScopeTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ScopeType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find scopetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ScopeType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all scopetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count scopetypes with conditions" })
  @ApiResponse({ status: 200, type: ScopeTypesResponse<ScopeType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ScopeTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one scopetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one scopetype or return error" })
  @ApiResponse({ status: 200, type: ScopeTypeResponse<ScopeType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ScopeType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get scopetype by ID" })
  @ApiResponse({ status: 200, type: ScopeTypeResponse<ScopeType> })
  @ApiResponse({ status: 404, description: "ScopeType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the scopetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ScopeTypeResponse<ScopeType>> {
    try {
      const scopetype = await this.service.findOne({ where: { id } });
      if (!scopetype) {
        throw new NotFoundException(
          "ScopeType no encontrado para el id solicitado"
        );
      }
      return scopetype;
    } catch (error) {
