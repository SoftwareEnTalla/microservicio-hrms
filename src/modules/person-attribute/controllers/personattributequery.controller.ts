...
import { PersonAttributeDto } from "../dtos/all-dto";
...
@Controller("personattributes/query")
export class PersonAttributeQueryController {
  #logger = new Logger(PersonAttributeQueryController.name);

  constructor(private readonly service: PersonAttributeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all personattribute with optional pagination" })
  @ApiResponse({ status: 200, type: PersonAttributesResponse })
  @ApiQuery({ name: "options", required: false, type: PersonAttributeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PersonAttribute>    
  ): Promise<PersonAttributesResponse<PersonAttribute>> {
    try {
     
      const personattributes = await this.service.findAll(options);
      logger.info("Retrieving all personattribute");
  ...
  @ApiParam({ name: 'field', required: true, description: 'Field to filter personattribute', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PersonAttributesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PersonAttribute no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find personattributes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PersonAttribute>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all personattributes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count personattributes with conditions" })
  @ApiResponse({ status: 200, type: PersonAttributesResponse<PersonAttribute> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PersonAttributes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one personattribute with conditions" })
  ...
  @ApiOperation({ summary: "Find one personattribute or return error" })
  @ApiResponse({ status: 200, type: PersonAttributeResponse<PersonAttribute> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
