...
import { TrainingDto } from "../dtos/all-dto";
...
@ApiTags("Training Query")
@UseGuards(TrainingAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("trainings/query")
export class TrainingQueryController {
  #logger = new Logger(TrainingQueryController.name);

  constructor(private readonly service: TrainingQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all training with optional pagination" })
  @ApiResponse({ status: 200, type: TrainingsResponse })
  @ApiQuery({ name: "options", required: false, type: TrainingDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Training>    
  ): Promise<TrainingsResponse<Training>> {
    try {
     
      const trainings = await this.service.findAll(options);
      logger.info("Retrieving all training");
      return trainings;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find training by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter training', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: TrainingsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Training no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find trainings with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Training>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all trainings" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count trainings with conditions" })
  @ApiResponse({ status: 200, type: TrainingsResponse<Training> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Trainings no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one training with conditions" })
  ...
  @ApiOperation({ summary: "Find one training or return error" })
  @ApiResponse({ status: 200, type: TrainingResponse<Training> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Training no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the training to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<TrainingResponse<Training>> {
    try {
      const training = await this.service.findOne({ where: { id } });
      if (!training) {
        throw new NotFoundException(
          "Training no encontrado para el id solicitado"
        );
      }
      return training;
    } catch (error) {
