...
@ApiTags("Reports Query")
@UseGuards(ReportsAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("reportss/query")
export class ReportsQueryController {
  #logger = new Logger(ReportsQueryController.name);

  constructor(private readonly service: ReportsQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all reports with optional pagination" })
  @ApiResponse({ status: 200, type: ReportssResponse })
  @ApiQuery({ name: "options", required: false, type: ReportsDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Reports>    
  ): Promise<ReportssResponse<Reports>> {
    try {
     
      const reportss = await this.service.findAll(options);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the reports to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ReportsResponse<Reports>> {
    try {
      const reports = await this.service.findOne({ where: { id } });
      if (!reports) {
        throw new NotFoundException(
          "Reports no encontrado para el id solicitado"
        );
      }
      return reports;
    } catch (error) {
  ...
  @ApiOperation({ summary: "Find reports by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter reports', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ReportssResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Reports no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find reportss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Reports>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all reportss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count reportss with conditions" })
  @ApiResponse({ status: 200, type: ReportssResponse<Reports> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Reportss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one reports with conditions" })
  @ApiResponse({ status: 200, type: ReportsResponse<Reports> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one reports or return error" })
  @ApiResponse({ status: 200, type: ReportsResponse<Reports> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Reports no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
