...
@ApiTags("ConfigurationParameterScope Query")
@UseGuards(ConfigurationParameterScopeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("configurationparameterscopes/query")
export class ConfigurationParameterScopeQueryController {
  #logger = new Logger(ConfigurationParameterScopeQueryController.name);

  constructor(private readonly service: ConfigurationParameterScopeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all configurationparameterscope with optional pagination" })
  @ApiResponse({ status: 200, type: ConfigurationParameterScopesResponse })
  @ApiQuery({ name: "options", required: false, type: ConfigurationParameterScopeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ConfigurationParameterScope>    
  ): Promise<ConfigurationParameterScopesResponse<ConfigurationParameterScope>> {
    try {
     
      const configurationparameterscopes = await this.service.findAll(options);
      logger.info("Retrieving all configurationparameterscope");
  ...
  @ApiOperation({ summary: "Find configurationparameterscope by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter configurationparameterscope', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ConfigurationParameterScopesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ConfigurationParameterScope no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find configurationparameterscopes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ConfigurationParameterScope>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all configurationparameterscopes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count configurationparameterscopes with conditions" })
  @ApiResponse({ status: 200, type: ConfigurationParameterScopesResponse<ConfigurationParameterScope> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ConfigurationParameterScopes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one configurationparameterscope with conditions" })
  ...
  @ApiOperation({ summary: "Find one configurationparameterscope or return error" })
  @ApiResponse({ status: 200, type: ConfigurationParameterScopeResponse<ConfigurationParameterScope> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ConfigurationParameterScope no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get configurationparameterscope by ID" })
  @ApiResponse({ status: 200, type: ConfigurationParameterScopeResponse<ConfigurationParameterScope> })
  @ApiResponse({ status: 404, description: "ConfigurationParameterScope not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the configurationparameterscope to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ConfigurationParameterScopeResponse<ConfigurationParameterScope>> {
    try {
      const configurationparameterscope = await this.service.findOne({ where: { id } });
      if (!configurationparameterscope) {
        throw new NotFoundException(
          "ConfigurationParameterScope no encontrado para el id solicitado"
        );
      }
      return configurationparameterscope;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
