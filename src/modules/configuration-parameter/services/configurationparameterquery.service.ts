...
@Injectable()
export class ConfigurationParameterQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(ConfigurationParameterQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: ConfigurationParameterQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${ConfigurationParameter.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const configurationparameters = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de configurationparameters obtenido con éxito",
        data: configurationparameters,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          configurationparameters.length
        ),
        count: configurationparameters.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<ConfigurationParameterResponse<ConfigurationParameter>> {
    try {
      const configurationparameter = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "ConfigurationParameters no encontrados para la propiedad y valor especificado"
        );
      // Devolver configurationparameter
      return {
        ok: true,
        message: "ConfigurationParameters obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<ConfigurationParameterResponse<ConfigurationParameter>> {
    try {
