...
@ApiTags("Person Query")
@UseGuards(PersonAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("persons/query")
export class PersonQueryController {
  #logger = new Logger(PersonQueryController.name);

  constructor(private readonly service: PersonQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all person with optional pagination" })
  @ApiResponse({ status: 200, type: PersonsResponse })
  @ApiQuery({ name: "options", required: false, type: PersonDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Person>    
  ): Promise<PersonsResponse<Person>> {
    try {
     
      const persons = await this.service.findAll(options);
  ...
  @ApiOperation({ summary: "Find person by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter person', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PersonsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Person no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find persons with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Person>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all persons" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count persons with conditions" })
  @ApiResponse({ status: 200, type: PersonsResponse<Person> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Persons no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one person with conditions" })
  ...
  @ApiOperation({ summary: "Find one person or return error" })
  @ApiResponse({ status: 200, type: PersonResponse<Person> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<PersonResponse<Person> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Person no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the person to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PersonResponse<Person>> {
    try {
      const person = await this.service.findOne({ where: { id } });
      if (!person) {
        throw new NotFoundException(
          "Person no encontrado para el id solicitado"
        );
      }
      return person;
    } catch (error) {
