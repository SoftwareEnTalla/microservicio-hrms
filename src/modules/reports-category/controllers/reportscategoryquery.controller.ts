...
import { ReportsCategoryDto } from "../dtos/all-dto";
...
@ApiTags("ReportsCategory Query")
@UseGuards(ReportsCategoryAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("reportscategorys/query")
export class ReportsCategoryQueryController {
  #logger = new Logger(ReportsCategoryQueryController.name);

  constructor(private readonly service: ReportsCategoryQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all reportscategory with optional pagination" })
  @ApiResponse({ status: 200, type: ReportsCategorysResponse })
  @ApiQuery({ name: "options", required: false, type: ReportsCategoryDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ReportsCategory>    
  ): Promise<ReportsCategorysResponse<ReportsCategory>> {
    try {
     
      const reportscategorys = await this.service.findAll(options);
      logger.info("Retrieving all reportscategory");
  ...
  @ApiOperation({ summary: "Find reportscategory by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter reportscategory', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ReportsCategorysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ReportsCategory no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find reportscategorys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ReportsCategory>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all reportscategorys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count reportscategorys with conditions" })
  @ApiResponse({ status: 200, type: ReportsCategorysResponse<ReportsCategory> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ReportsCategorys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one reportscategory with conditions" })
  ...
  @ApiOperation({ summary: "Find one reportscategory or return error" })
  @ApiResponse({ status: 200, type: ReportsCategoryResponse<ReportsCategory> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ReportsCategory no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get reportscategory by ID" })
  @ApiResponse({ status: 200, type: ReportsCategoryResponse<ReportsCategory> })
  @ApiResponse({ status: 404, description: "ReportsCategory not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the reportscategory to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ReportsCategoryResponse<ReportsCategory>> {
    try {
      const reportscategory = await this.service.findOne({ where: { id } });
      if (!reportscategory) {
        throw new NotFoundException(
          "ReportsCategory no encontrado para el id solicitado"
        );
      }
