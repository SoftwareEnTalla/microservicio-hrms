...
@ApiTags("CredentialStatus Query")
@UseGuards(CredentialStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("credentialstatuss/query")
export class CredentialStatusQueryController {
  #logger = new Logger(CredentialStatusQueryController.name);

  constructor(private readonly service: CredentialStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all credentialstatus with optional pagination" })
  @ApiResponse({ status: 200, type: CredentialStatussResponse })
  @ApiQuery({ name: "options", required: false, type: CredentialStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CredentialStatus>    
  ): Promise<CredentialStatussResponse<CredentialStatus>> {
    try {
     
      const credentialstatuss = await this.service.findAll(options);
      logger.info("Retrieving all credentialstatus");
  ...
  @ApiOperation({ summary: "Find credentialstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter credentialstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CredentialStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CredentialStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find credentialstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CredentialStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all credentialstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count credentialstatuss with conditions" })
  @ApiResponse({ status: 200, type: CredentialStatussResponse<CredentialStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one credentialstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one credentialstatus or return error" })
  @ApiResponse({ status: 200, type: CredentialStatusResponse<CredentialStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get credentialstatus by ID" })
  @ApiResponse({ status: 200, type: CredentialStatusResponse<CredentialStatus> })
  @ApiResponse({ status: 404, description: "CredentialStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the credentialstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CredentialStatusResponse<CredentialStatus>> {
    try {
      const credentialstatus = await this.service.findOne({ where: { id } });
      if (!credentialstatus) {
        throw new NotFoundException(
