...
@Injectable()
export class LeaveRequestQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(LeaveRequestQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: LeaveRequestQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${LeaveRequest.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const leaverequests = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de leaverequests obtenido con éxito",
        data: leaverequests,
  ...
  async findById(id: string): Promise<LeaveRequestResponse<LeaveRequest>> {
    try {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "LeaveRequests no encontrados para la propiedad y valor especificado"
        );
      // Devolver leaverequest
      return {
        ok: true,
        message: "LeaveRequests obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<LeaveRequestResponse<LeaveRequest>> {
    try {
