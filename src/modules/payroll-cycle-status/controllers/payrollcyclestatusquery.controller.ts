...
@ApiTags("PayrollCycleStatus Query")
@UseGuards(PayrollCycleStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("payrollcyclestatuss/query")
export class PayrollCycleStatusQueryController {
  #logger = new Logger(PayrollCycleStatusQueryController.name);

  constructor(private readonly service: PayrollCycleStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all payrollcyclestatus with optional pagination" })
  @ApiResponse({ status: 200, type: PayrollCycleStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PayrollCycleStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PayrollCycleStatus>    
  ): Promise<PayrollCycleStatussResponse<PayrollCycleStatus>> {
    try {
     
      const payrollcyclestatuss = await this.service.findAll(options);
      logger.info("Retrieving all payrollcyclestatus");
  ...
  @ApiOperation({ summary: "Find payrollcyclestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter payrollcyclestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PayrollCycleStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PayrollCycleStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find payrollcyclestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PayrollCycleStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all payrollcyclestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count payrollcyclestatuss with conditions" })
  @ApiResponse({ status: 200, type: PayrollCycleStatussResponse<PayrollCycleStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PayrollCycleStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one payrollcyclestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one payrollcyclestatus or return error" })
  @ApiResponse({ status: 200, type: PayrollCycleStatusResponse<PayrollCycleStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PayrollCycleStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get payrollcyclestatus by ID" })
  @ApiResponse({ status: 200, type: PayrollCycleStatusResponse<PayrollCycleStatus> })
  @ApiResponse({ status: 404, description: "PayrollCycleStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the payrollcyclestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PayrollCycleStatusResponse<PayrollCycleStatus>> {
    try {
      const payrollcyclestatus = await this.service.findOne({ where: { id } });
      if (!payrollcyclestatus) {
        throw new NotFoundException(
          "PayrollCycleStatus no encontrado para el id solicitado"
        );
      }
      return payrollcyclestatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
