...
import { AttendanceChannelDto } from "../dtos/all-dto";
...
@ApiTags("AttendanceChannel Query")
@UseGuards(AttendanceChannelAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("attendancechannels/query")
export class AttendanceChannelQueryController {
  #logger = new Logger(AttendanceChannelQueryController.name);

  constructor(private readonly service: AttendanceChannelQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all attendancechannel with optional pagination" })
  @ApiResponse({ status: 200, type: AttendanceChannelsResponse })
  @ApiQuery({ name: "options", required: false, type: AttendanceChannelDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AttendanceChannel>    
  ): Promise<AttendanceChannelsResponse<AttendanceChannel>> {
    try {
     
      const attendancechannels = await this.service.findAll(options);
      logger.info("Retrieving all attendancechannel");
  ...
  @ApiOperation({ summary: "Find attendancechannel by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter attendancechannel', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AttendanceChannelsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AttendanceChannel no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find attendancechannels with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AttendanceChannel>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all attendancechannels" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count attendancechannels with conditions" })
  @ApiResponse({ status: 200, type: AttendanceChannelsResponse<AttendanceChannel> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades AttendanceChannels no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one attendancechannel with conditions" })
  ...
  @ApiOperation({ summary: "Find one attendancechannel or return error" })
  @ApiResponse({ status: 200, type: AttendanceChannelResponse<AttendanceChannel> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad AttendanceChannel no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get attendancechannel by ID" })
  @ApiResponse({ status: 200, type: AttendanceChannelResponse<AttendanceChannel> })
  @ApiResponse({ status: 404, description: "AttendanceChannel not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the attendancechannel to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AttendanceChannelResponse<AttendanceChannel>> {
    try {
      const attendancechannel = await this.service.findOne({ where: { id } });
      if (!attendancechannel) {
        throw new NotFoundException(
          "AttendanceChannel no encontrado para el id solicitado"
        );
      }
