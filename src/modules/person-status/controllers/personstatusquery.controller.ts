...
@ApiTags("PersonStatus Query")
@UseGuards(PersonStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("personstatuss/query")
export class PersonStatusQueryController {
  #logger = new Logger(PersonStatusQueryController.name);

  constructor(private readonly service: PersonStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all personstatus with optional pagination" })
  @ApiResponse({ status: 200, type: PersonStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PersonStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PersonStatus>    
  ): Promise<PersonStatussResponse<PersonStatus>> {
    try {
     
      const personstatuss = await this.service.findAll(options);
      logger.info("Retrieving all personstatus");
  ...
  @ApiOperation({ summary: "Find personstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter personstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PersonStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PersonStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find personstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PersonStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all personstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count personstatuss with conditions" })
  @ApiResponse({ status: 200, type: PersonStatussResponse<PersonStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PersonStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one personstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one personstatus or return error" })
  @ApiResponse({ status: 200, type: PersonStatusResponse<PersonStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PersonStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get personstatus by ID" })
  @ApiResponse({ status: 200, type: PersonStatusResponse<PersonStatus> })
  @ApiResponse({ status: 404, description: "PersonStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the personstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PersonStatusResponse<PersonStatus>> {
    try {
      const personstatus = await this.service.findOne({ where: { id } });
      if (!personstatus) {
        throw new NotFoundException(
          "PersonStatus no encontrado para el id solicitado"
        );
      }
      return personstatus;
    } catch (error) {
