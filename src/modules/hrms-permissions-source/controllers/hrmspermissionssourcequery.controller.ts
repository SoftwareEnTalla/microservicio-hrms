...
import { HrmsPermissionsSourceDto } from "../dtos/all-dto";
...
@ApiTags("HrmsPermissionsSource Query")
@UseGuards(HrmsPermissionsSourceAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("hrmspermissionssources/query")
export class HrmsPermissionsSourceQueryController {
  #logger = new Logger(HrmsPermissionsSourceQueryController.name);

  constructor(private readonly service: HrmsPermissionsSourceQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all hrmspermissionssource with optional pagination" })
  @ApiResponse({ status: 200, type: HrmsPermissionsSourcesResponse })
  @ApiQuery({ name: "options", required: false, type: HrmsPermissionsSourceDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<HrmsPermissionsSource>    
  ): Promise<HrmsPermissionsSourcesResponse<HrmsPermissionsSource>> {
    try {
     
      const hrmspermissionssources = await this.service.findAll(options);
      logger.info("Retrieving all hrmspermissionssource");
  ...
  @ApiOperation({ summary: "Find hrmspermissionssource by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter hrmspermissionssource', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: HrmsPermissionsSourcesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "HrmsPermissionsSource no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find hrmspermissionssources with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<HrmsPermissionsSource>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all hrmspermissionssources" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count hrmspermissionssources with conditions" })
  @ApiResponse({ status: 200, type: HrmsPermissionsSourcesResponse<HrmsPermissionsSource> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades HrmsPermissionsSources no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one hrmspermissionssource with conditions" })
  ...
  @ApiOperation({ summary: "Find one hrmspermissionssource or return error" })
  @ApiResponse({ status: 200, type: HrmsPermissionsSourceResponse<HrmsPermissionsSource> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad HrmsPermissionsSource no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get hrmspermissionssource by ID" })
  @ApiResponse({ status: 200, type: HrmsPermissionsSourceResponse<HrmsPermissionsSource> })
  @ApiResponse({ status: 404, description: "HrmsPermissionsSource not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the hrmspermissionssource to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<HrmsPermissionsSourceResponse<HrmsPermissionsSource>> {
    try {
      const hrmspermissionssource = await this.service.findOne({ where: { id } });
      if (!hrmspermissionssource) {
        throw new NotFoundException(
          "HrmsPermissionsSource no encontrado para el id solicitado"
        );
      }
      return hrmspermissionssource;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
