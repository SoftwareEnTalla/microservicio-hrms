...
import { GenderDto } from "../dtos/all-dto";
...
@ApiTags("Gender Query")
@UseGuards(GenderAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("genders/query")
export class GenderQueryController {
  #logger = new Logger(GenderQueryController.name);

  constructor(private readonly service: GenderQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all gender with optional pagination" })
  @ApiResponse({ status: 200, type: GendersResponse })
  @ApiQuery({ name: "options", required: false, type: GenderDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Gender>    
  ): Promise<GendersResponse<Gender>> {
    try {
     
      const genders = await this.service.findAll(options);
      logger.info("Retrieving all gender");
      return genders;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find gender by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter gender', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: GendersResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Gender no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find genders with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Gender>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all genders" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count genders with conditions" })
  @ApiResponse({ status: 200, type: GendersResponse<Gender> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Genders no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one gender with conditions" })
  ...
  @ApiOperation({ summary: "Find one gender or return error" })
  @ApiResponse({ status: 200, type: GenderResponse<Gender> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Gender no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the gender to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<GenderResponse<Gender>> {
    try {
      const gender = await this.service.findOne({ where: { id } });
      if (!gender) {
        throw new NotFoundException(
          "Gender no encontrado para el id solicitado"
        );
      }
      return gender;
    } catch (error) {
