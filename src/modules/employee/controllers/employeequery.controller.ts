...
import { EmployeeDto } from "../dtos/all-dto";
...
@ApiTags("Employee Query")
@UseGuards(EmployeeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("employees/query")
export class EmployeeQueryController {
  #logger = new Logger(EmployeeQueryController.name);

  constructor(private readonly service: EmployeeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all employee with optional pagination" })
  @ApiResponse({ status: 200, type: EmployeesResponse })
  @ApiQuery({ name: "options", required: false, type: EmployeeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Employee>    
  ): Promise<EmployeesResponse<Employee>> {
    try {
     
      const employees = await this.service.findAll(options);
  ...
  @ApiOperation({ summary: "Find employee by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter employee', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: EmployeesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Employee no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find employees with pagination" })
  @ApiResponse({ status: 200, type: EmployeesResponse<Employee> })
  @ApiQuery({ name: "options", required: false, type: EmployeeDto }) // Ajustar según el tipo real
  @ApiQuery({ name: "page", required: false, type: Number })
  @ApiQuery({ name: "size", required: false, type: Number })
  @ApiQuery({ name: "sort", required: false, type: String })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Employee>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all employees" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count employees with conditions" })
  @ApiResponse({ status: 200, type: EmployeesResponse<Employee> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Employees no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one employee with conditions" })
  @ApiResponse({ status: 200, type: EmployeeResponse<Employee> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  @ApiOperation({ summary: "Find one employee or return error" })
  @ApiResponse({ status: 200, type: EmployeeResponse<Employee> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Employee no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the employee to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<EmployeeResponse<Employee>> {
    try {
      const employee = await this.service.findOne({ where: { id } });
      if (!employee) {
        throw new NotFoundException(
          "Employee no encontrado para el id solicitado"
        );
      }
      return employee;
    } catch (error) {
