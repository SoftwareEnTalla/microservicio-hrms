...
import { AttendanceStatusDto } from "../dtos/all-dto";
...
@ApiTags("AttendanceStatus Query")
@UseGuards(AttendanceStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("attendancestatuss/query")
export class AttendanceStatusQueryController {
  #logger = new Logger(AttendanceStatusQueryController.name);

  constructor(private readonly service: AttendanceStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all attendancestatus with optional pagination" })
  @ApiResponse({ status: 200, type: AttendanceStatussResponse })
  @ApiQuery({ name: "options", required: false, type: AttendanceStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AttendanceStatus>    
  ): Promise<AttendanceStatussResponse<AttendanceStatus>> {
    try {
     
      const attendancestatuss = await this.service.findAll(options);
      logger.info("Retrieving all attendancestatus");
  ...
  @ApiOperation({ summary: "Find attendancestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter attendancestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AttendanceStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AttendanceStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find attendancestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AttendanceStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all attendancestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count attendancestatuss with conditions" })
  @ApiResponse({ status: 200, type: AttendanceStatussResponse<AttendanceStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one attendancestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one attendancestatus or return error" })
  @ApiResponse({ status: 200, type: AttendanceStatusResponse<AttendanceStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get attendancestatus by ID" })
  @ApiResponse({ status: 200, type: AttendanceStatusResponse<AttendanceStatus> })
  @ApiResponse({ status: 404, description: "AttendanceStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the attendancestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AttendanceStatusResponse<AttendanceStatus>> {
    try {
      const attendancestatus = await this.service.findOne({ where: { id } });
      if (!attendancestatus) {
        throw new NotFoundException(
