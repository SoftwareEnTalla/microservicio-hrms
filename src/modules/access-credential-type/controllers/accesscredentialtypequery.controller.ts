...
@ApiTags("AccessCredentialType Query")
@UseGuards(AccessCredentialTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("accesscredentialtypes/query")
export class AccessCredentialTypeQueryController {
  #logger = new Logger(AccessCredentialTypeQueryController.name);

  constructor(private readonly service: AccessCredentialTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all accesscredentialtype with optional pagination" })
  @ApiResponse({ status: 200, type: AccessCredentialTypesResponse })
  @ApiQuery({ name: "options", required: false, type: AccessCredentialTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<AccessCredentialType>    
  ): Promise<AccessCredentialTypesResponse<AccessCredentialType>> {
    try {
     
      const accesscredentialtypes = await this.service.findAll(options);
      logger.info("Retrieving all accesscredentialtype");
  ...
  @ApiOperation({ summary: "Find accesscredentialtype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter accesscredentialtype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AccessCredentialTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "AccessCredentialType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find accesscredentialtypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<AccessCredentialType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all accesscredentialtypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count accesscredentialtypes with conditions" })
  @ApiResponse({ status: 200, type: AccessCredentialTypesResponse<AccessCredentialType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one accesscredentialtype with conditions" })
  ...
  @ApiOperation({ summary: "Find one accesscredentialtype or return error" })
  @ApiResponse({ status: 200, type: AccessCredentialTypeResponse<AccessCredentialType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get accesscredentialtype by ID" })
  @ApiResponse({ status: 200, type: AccessCredentialTypeResponse<AccessCredentialType> })
  @ApiResponse({ status: 404, description: "AccessCredentialType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the accesscredentialtype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AccessCredentialTypeResponse<AccessCredentialType>> {
    try {
      const accesscredentialtype = await this.service.findOne({ where: { id } });
