...
@ApiTags("TrainingModality Query")
@UseGuards(TrainingModalityAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("trainingmodalitys/query")
export class TrainingModalityQueryController {
  #logger = new Logger(TrainingModalityQueryController.name);

  constructor(private readonly service: TrainingModalityQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all trainingmodality with optional pagination" })
  @ApiResponse({ status: 200, type: TrainingModalitysResponse })
  @ApiQuery({ name: "options", required: false, type: TrainingModalityDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<TrainingModality>    
  ): Promise<TrainingModalitysResponse<TrainingModality>> {
    try {
     
      const trainingmodalitys = await this.service.findAll(options);
      logger.info("Retrieving all trainingmodality");
  ...
  @ApiOperation({ summary: "Find trainingmodality by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter trainingmodality', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: TrainingModalitysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "TrainingModality no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find trainingmodalitys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<TrainingModality>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all trainingmodalitys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count trainingmodalitys with conditions" })
  @ApiResponse({ status: 200, type: TrainingModalitysResponse<TrainingModality> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one trainingmodality with conditions" })
  ...
  @ApiOperation({ summary: "Find one trainingmodality or return error" })
  @ApiResponse({ status: 200, type: TrainingModalityResponse<TrainingModality> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get trainingmodality by ID" })
  @ApiResponse({ status: 200, type: TrainingModalityResponse<TrainingModality> })
  @ApiResponse({ status: 404, description: "TrainingModality not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the trainingmodality to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<TrainingModalityResponse<TrainingModality>> {
    try {
      const trainingmodality = await this.service.findOne({ where: { id } });
      if (!trainingmodality) {
        throw new NotFoundException(
