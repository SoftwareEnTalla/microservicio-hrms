...
import { LastAccessOutcomeDto } from "../dtos/all-dto";
...
@ApiTags("LastAccessOutcome Query")
@UseGuards(LastAccessOutcomeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("lastaccessoutcomes/query")
export class LastAccessOutcomeQueryController {
  #logger = new Logger(LastAccessOutcomeQueryController.name);

  constructor(private readonly service: LastAccessOutcomeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all lastaccessoutcome with optional pagination" })
  @ApiResponse({ status: 200, type: LastAccessOutcomesResponse })
  @ApiQuery({ name: "options", required: false, type: LastAccessOutcomeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<LastAccessOutcome>    
  ): Promise<LastAccessOutcomesResponse<LastAccessOutcome>> {
    try {
     
      const lastaccessoutcomes = await this.service.findAll(options);
      logger.info("Retrieving all lastaccessoutcome");
  ...
  @ApiOperation({ summary: "Find lastaccessoutcome by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter lastaccessoutcome', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LastAccessOutcomesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "LastAccessOutcome no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find lastaccessoutcomes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<LastAccessOutcome>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all lastaccessoutcomes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count lastaccessoutcomes with conditions" })
  @ApiResponse({ status: 200, type: LastAccessOutcomesResponse<LastAccessOutcome> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one lastaccessoutcome with conditions" })
  ...
  @ApiOperation({ summary: "Find one lastaccessoutcome or return error" })
  @ApiResponse({ status: 200, type: LastAccessOutcomeResponse<LastAccessOutcome> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get lastaccessoutcome by ID" })
  @ApiResponse({ status: 200, type: LastAccessOutcomeResponse<LastAccessOutcome> })
  @ApiResponse({ status: 404, description: "LastAccessOutcome not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the lastaccessoutcome to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LastAccessOutcomeResponse<LastAccessOutcome>> {
    try {
      const lastaccessoutcome = await this.service.findOne({ where: { id } });
      if (!lastaccessoutcome) {
        throw new NotFoundException(
          "LastAccessOutcome no encontrado para el id solicitado"
        );
      }
      return lastaccessoutcome;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
