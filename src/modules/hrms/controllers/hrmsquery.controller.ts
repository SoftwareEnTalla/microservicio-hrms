...
import { HrmsDto } from "../dtos/all-dto";
...
@Controller("hrmss/query")
export class HrmsQueryController {
  #logger = new Logger(HrmsQueryController.name);

  constructor(private readonly service: HrmsQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all hrms with optional pagination" })
  @ApiResponse({ status: 200, type: HrmssResponse })
  @ApiQuery({ name: "options", required: false, type: HrmsDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Hrms>    
  ): Promise<HrmssResponse<Hrms>> {
    try {
     
      const hrmss = await this.service.findAll(options);
      logger.info("Retrieving all hrms");
      return hrmss;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find hrms by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter hrms', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: HrmssResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Hrms no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find hrmss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Hrms>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all hrmss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count hrmss with conditions" })
  @ApiResponse({ status: 200, type: HrmssResponse<Hrms> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Hrmss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one hrms with conditions" })
  ...
  @ApiOperation({ summary: "Find one hrms or return error" })
  @ApiResponse({ status: 200, type: HrmsResponse<Hrms> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Hrms no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get hrms by ID" })
  @ApiResponse({ status: 200, type: HrmsResponse<Hrms> })
  @ApiResponse({ status: 404, description: "Hrms not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the hrms to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<HrmsResponse<Hrms>> {
    try {
      const hrms = await this.service.findOne({ where: { id } });
      if (!hrms) {
        throw new NotFoundException(
          "Hrms no encontrado para el id solicitado"
        );
      }
      return hrms;
    } catch (error) {
